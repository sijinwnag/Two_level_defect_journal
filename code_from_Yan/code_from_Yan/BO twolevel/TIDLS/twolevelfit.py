...
filelist = ['n30C.xlsm', 'n70C.xlsm', 'p30C.xlsm']
Doplist = []
Tlist = []
Typelist=[]
nxclist=[]
...
for Et1 in Et1list:
    for Et2 in Et2list:
        ...
        def squareresidual(siglist, **kwarg):
            ...
            fraction = siglist[4]
            squareerror = 0
            for T, nxc, ve, vh, ni, n, p, taum, pp in zip(Tlist, nxclist, velist, vhlist, nilist, nlist, plist, taulist,[1,1,fraction*100]):
                n1 = ni * np.exp(Et1 / kb / T)
                p1 = ni * np.exp(-Et1 / kb / T)
                n2 = ni * np.exp(Et2 / kb / T)
                p2 = ni * np.exp(-Et2 / kb / T)
...
for taum, T, Ndop, doptype, nxc, p in zip(taulist, Tlist,Doplist,Typelist, nxclist, [1,1,scaleopt]):
