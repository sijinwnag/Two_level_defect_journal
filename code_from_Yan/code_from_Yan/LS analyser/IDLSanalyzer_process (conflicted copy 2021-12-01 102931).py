...
class LSana(QMainWindow, Ui_IDLSanalyzer):
    ...
    def __init__(self, parent=None):
        ...
        self.pushButton_exp.setEnabled(False)
        self.pushButton_exp.clicked.connect(self.expassinton)
            ...
            for fname in filename[0]:
                if os.path.splitext(fname)[1] == '.txt':
                    ...
                    for col in ws.iter_cols(min_col=4, max_col=10, min_row=1,max_row=1):
                        if col[0].value == 'Tau (sec)' or col[0].value == 'Tau (s)':
                            taucol = col[0].column
                            break
                    for col in ws.iter_cols(min_col=4, max_col=10, min_row=1,max_row=1):
                        if col[0].value == 'Minority Carrier Density':
                            mcdcol = col[0].column
                            break
                    for row in ws.iter_rows(min_row=5, max_row=5+lenth, min_col=taucol, max_col=taucol):
                        if row[0].value is not None and row[-1].value is not None:
                            tau.append(row[0].value)
                    for row in ws.iter_rows(min_row=5, max_row=5+lenth, min_col=mcdcol, max_col=mcdcol):
                        if row[0].value is not None and row[-1].value is not None:
    ...
    def checkselection(self):
        ...
        if len(self.listWidget_analysis.selectedItems()) > 0:
            ...
            self.pushButton_exp.setEnabled(True)
        else:
            self.pushButton_delanalysis.setEnabled(False)
            self.pushButton_viewset.setEnabled(False)
            self.pushButton_exp.setEnabled(False)
                ...
                if data.uid == item.data(32) and data.checkset() == True:
                    ...
                    if data.doptype == 'n':
                        Nd = Ion(temp=data.temp, ni_author=ni_author).update_dopant_ionisation(
                            author=ionauthor, N_dop=data.Ndop, nxc=data.nxc, impurity='phosphorous')
                        Na = 1
                    if data.doptype == 'p':
                        Na = Ion(temp=data.temp, ni_author=ni_author).update_dopant_ionisation(
                            author=ionauthor, N_dop=data.Ndop, nxc=data.nxc, impurity='boron')
                        Nd = 1
        ...
        for item in self.listWidget_analysis.selectedItems():
            for data in self.Rawdata:
                if data.uid == item.data(32):
                    ...
                    sheet2['A6'].value = data.name
                    sheet2['J9'].value = data.Ndop
                    sheet2['D6'].value = data.doptype
                    if data.j0 is not None:
                        sheet2['D9'].value = data.j0
                    if data.thickness is not None:
                        sheet2['B6'].value = data.thickness
                    sheet2['L9'].value = data.temp - 273.15
                    for i in range(len(data.nxc)):
                        sheet1.cell(row=5 + i, column=7).value = data.nxc[i]
                        sheet1.cell(row=5 + i, column=5).value = data.tau[i]
                    fname = os.path.join(
                        path, data.name + '_fake.xlsm')
                    book.save(fname)
