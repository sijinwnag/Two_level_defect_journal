...
class Defect():
    ...
    def __init__(self,Et,Sn,Sp,Nt=None, name="", Capture_mode_n=None,Capture_mode_p=None, Capture_param_n=None, Capture_param_p=None):
        ...
        self.name=name
    def randomDB(N,Et_min = None, Et_max = None, S_min_n = None, S_max_n = None, S_min_p = None, S_max_p = None, Nt = None,CMn_tab=None,CMp_tab=None,Force_same_CM=False):
        '''
        ---Doc---
            Description:
            ...
            Inputs:
                ...
                S_min_n       Float       Lower boundary for Capture Cross-Section for electron
                S_max_n       Float       Higher boundary for Capture Cross-Section for electron
                S_min_p      Float       Lower boundary for Capture Cross-Section for hole
                S_max_p       Float       Higher boundary for Capture Cross-Section for hole
        ...
        if S_min_n is None: S_min_n =  Defect.Sminbound
        if S_max_n is None: S_max_n =  Defect.Smaxbound
        if S_min_p is None: S_min_p =  Defect.Sminbound
        if S_max_p is None: S_max_p =  Defect.Smaxbound
        if Nt is None: Nt = Defect.DefaultNt
        ...
        for i in range(N):
            ...
            Sn = np.exp(np.random.rand(1)[0]*(np.log(S_max_n)-np.log(S_min_n))+np.log(S_min_n))
            Sp= np.exp(np.random.rand(1)[0]*(np.log(S_max_p)-np.log(S_min_p))+np.log(S_min_p))
