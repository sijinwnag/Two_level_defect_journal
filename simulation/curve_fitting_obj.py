...
class experiment_fitter():
    ...
    def __init__(self):
        ...
        '''
        self.SAVEDIR = r"D:\study\thesis_data_storage\journal\defect_classification\basic_model\testing_data"
    ...
    def two_level_simulate_BO(self, emailme = False, three_curves = False):
        '''
        ...
        self.S_p_1 = 3.1622776601683794e-15
        self.S_p_2 = 5.623413251903491e-16
        self.k_1 = 12711
        self.k_2 = 1.184
        ...
        # 'S_max_2_n':1E-13,              # maximum capcture cross section for electron.
        # the parameter to simulate BO:
        'Et_min_1':-0.453,             #   Minimum defect energy level
        'Et_max_1':-0.453,              #   Maximum defect energy level
        'Et_min_2':0.306,             #   Minimum defect energy level
        'Et_max_2':0.306,              #   Maximum defect energy level
        'S_min_1_p':self.S_p_1,              #   Minimum capture cross section for hole.
        'S_min_1_n':self.S_p_1*self.k_1,          #   Minimum capture cross section for electron.
        'S_max_1_p':self.S_p_1,              #   Maximum capture cross section for hole.
        'S_max_1_n':self.S_p_1*self.k_1,              # maximum capcture cross section for electron.
        'S_min_2_p':self.S_p_2,              #   Minimum capture cross section for hole.
        'S_min_2_n':self.S_p_2*self.k_2,          #   Minimum capture cross section for electron.
        'S_max_2_p':self.S_p_2,              #   Maximum capture cross section for hole.
        'S_max_2_n':self.S_p_2*self.k_2,              # maximum capcture cross section for electron.
        'Nt':1e12,                  #   Defect density
        'check_auger': False,     #   Check wether to resample if lifetime is auger-limited
        'noise':'',             #   Enable noiseparam
        'noiseparam':0,         #   Adds noise proportional to the log of Delta n
        ...
        self.PARAM['Nt'] = 3162277660.1683793
        ...
        # Simualate datasets: for n type 343K and excess carrier concentration from 1e14 to 3e16
        # update the wafer type
        self.WAFERTYPE = 'n'
        self.PARAM['type'] = self.WAFERTYPE
        # update the excess carrier concentration
        ...
        self.PARAM['Nt'] = 316227766016.83795
        # update the excess carrier concentration
        self.PARAMETERS['dn_range'] = np.logspace(14,16.5,100)
        # simulate the data
        ...
        # when we want to use all the three lifetime curves
        if three_curves:
            ...
            plt.plot(self.dn_n_303_2e15,self.tau_n_303_2e15*1e6, label='303K, n-type, Yan data')
    ...
    def BO_training_simulator(self, emailme = False, lifeitme_curve=[0, 1 ,2]):
        '''
        This function simulate the lifetime for two-level defects specifically for BO

        lifetime_index: it is a list of index regarding which lifetime to simulate, for example, if [0, 2] then it means n at 303K at p at 303K only.
        '''
        #  Define experiment: every time remember to run this line to refresh the code.
        self.exp = Experiment(SaveDir=self.SAVEDIR, Parameters=self.PARAMETERS)
        ...
        # combine the three lifetime curves horizontally
        available_data = [dataDf_p_303, dataDf_n_343, dataDf_n_303]
        dataDf = pd.concat(available_data,axis=1)
        # remove the duplicated columns
        dataDf = dataDf.loc[:,~dataDf.columns.duplicated()]
        # upload the data
        self.exp.uploadDB(dataDf)
    ...
    def interpolate_data_exporter(self):
        ...
        interpolatedata = np.concatenate((self.tau_p_303_8_5e14_interpolated.reshape(1,100),  
                                            self.tau_p_343_2e15_interpolated.reshape(1,100),
    ...
    def experimental_data_fit(self, sp1_resolution=10, sp2_resolution=10 ,Ntp_resolution=10, Ntn_resolution=10, plot=True):
        ...
        # load interpolated lifetime data from the object.
        p_303K = self.tau_p_303_8_5e14_interpolated
        dn_p_303K = self.dn_p_303_8_5e14_interpolated
        n_303K = self.tau_n_303_2e15_interpolated
        dn_n_303K = self.dn_n_303_2e15_interpolated

        # define hte simulatoin parameters:
        ...
        sp1_range = np.logspace(-13.5, -14.5, sp1_resolution)
        sp2_range = np.logspace(-15.5, -14.5, sp2_resolution)
        # define the range for defect density
        Ntp_range = np.logspace(9.5, 10.5, Ntp_resolution)
        Ntn_range = np.logspace(11.5, 12.5, Ntn_resolution)

        # define into the object
        self.PARAM['Et_min_1'] = Et1
        self.PARAM['Et_max_1'] = Et1
        self.PARAM['Et_min_2'] = Et2
        self.PARAM['Et_max_2'] = Et2

        # loop through sp1 and sp2 option.
        sp1_opt = 0
        sp2_opt = 0
        loss_opt = 1e10
        counter = 0
        for sp1 in sp1_range:
            for sp2 in sp2_range:
                ...
                self.PARAM['S_min_1_n'] = sp1*k1
                self.PARAM['S_max_1_n'] = sp1*k1
                ...
                for Ntp in Ntp_range:
                    for Ntn in Ntn_range:
                        ...
                        curve_dist_p = np.sum(np.sum((dataDf_p_303-p_303K)**2/p_303K**2))
                        curve_dist_n = np.sum(np.sum((dataDf_n_303-n_303K)**2/n_303K**2))
    ...
    def SHR_objective_function(self, params):
        ...
        Et1, Et2, sp1, sp2, sn1, sn2, Ntp, Ntn, = params

        # Update parameters for simulatoion
        self.PARAM['S_min_1_p'] = sp1
        self.PARAM['S_max_1_p'] = sp1
        self.PARAM['S_min_2_p'] = sp2
        self.PARAM['S_max_2_p'] = sp2
        self.PARAM['S_min_1_n'] = sn1
        self.PARAM['S_max_1_n'] = sn1 
        self.PARAM['S_min_2_n'] = sn2
        self.PARAM['S_max_2_n'] = sn2
        self.PARAM['Et_min_1'] = Et1
        self.PARAM['Et_max_1'] = Et1
        self.PARAM['Et_min_2'] = Et2   
        self.PARAM['Et_max_2'] = Et2
        ...
        self.WAFERTYPE = 'p'
        self.PARAM['type'] = self.WAFERTYPE
        # update the defect concentration
        self.PARAM['Nt'] = Ntp
        self.PARAMETERS['dn_range'] = np.logspace(13, 15.5, 100)
        ...
        db_sah['Mode'] = ['Single two-level'] * len(db_sah)
        select_X_list = []
        for string in db_sah.columns.tolist():
            if string[0].isdigit():
                select_X_list.append(string)
        dataDf_p_303 = np.array(db_sah[select_X_list])[0]
        ...
        for string in db_sah.columns.tolist():
            if string[0].isdigit():
                select_X_list.append(string)
        dataDf_n_303 = np.array(db_sah[select_X_list])[0]
    ...
    def experimental_data_fit2(self, initial_guess=None):
        '''
        Minimize the lost function output by SHR_objective_function
        '''
        ...
        sn2_min = 1e-15
        sn2_max = 1e-13
        # defect concentratoins
        Ntp_min = 1e10
        Ntp_max = 1e10
        Ntn_min = 1e12
        Ntn_max = 1e12
        # write into bounds
        bounds = [(Et_min_1, Et_max_1), (Et_min_2, Et_max_2), (sp1_min, sp1_max), (sp2_min, sp2_max),(sn1_min, sn1_max), (sn2_min, sn2_max), (Ntp_min, Ntp_max), (Ntn_min, Ntn_max)]

        # define the initial guess
        if initial_guess == None:
            initial_guess = [-0.453, 0.306, 1e-14, 1e-14*112711, 1e-15, 1e-15*1.184, 1e10, 1e12]

        # minimize the loss function
        print('Start minimization')
        result = minimize(self.SHR_objective_function, x0=initial_guess, bounds=bounds, method='L-BFGS-B')

        print(result)

        # simulate the optimal result
        Et1_opt = result.x[0]
        Et2_opt = result.x[1]
        sp1_opt = result.x[2]
        sp2_opt = result.x[3]
        ...
        # Update parameters for simulatoion
        self.PARAM['S_min_1_p'] = sp1_opt
        self.PARAM['S_max_1_p'] = sp1_opt
        self.PARAM['S_min_2_p'] = sp2_opt
        self.PARAM['S_max_2_p'] = sp2_opt
        self.PARAM['S_min_1_n'] = sn1_opt
        self.PARAM['S_max_1_n'] = sn1_opt
        self.PARAM['S_min_2_n'] = sn2_opt
        self.PARAM['S_max_2_n'] = sn2_opt
        self.PARAM['Et_min_1'] = Et1_opt
        self.PARAM['Et_max_1'] = Et1_opt
        self.PARAM['Et_min_2'] = Et2_opt
        self.PARAM['Et_max_2'] = Et2_opt

        # Simulate the BO lifetime data: p 303K, 8.5e14.
        self.WAFERTYPE = 'p'
        self.PARAM['type'] = self.WAFERTYPE
        # update the defect concentration
        self.PARAM['Nt'] = Ntp_opt
        self.PARAMETERS['dn_range'] = np.logspace(13, 15.5, 100)
        db_sah = DPML.generateDB_sah(self.PARAMETERS['n_defects'], [303], [8.5e14], 
                                    np.logspace(13, 15.5, 100), self.PARAM)
        db_sah['Mode'] = ['Single two-level'] * len(db_sah)
        ...
        plt.scatter(self.dn_p_303_8_5e14, self.tau_p_303_8_5e14, label='p 303K, 8.5e14 experiment')
        plt.scatter(self.dn_n_303_2e15, self.tau_n_303_2e15, label='n 303K, 2e15 experiment')
        plt.plot(np.logspace(13, 15.5, 100), dataDf_p_303, label='p 303K, 8.5e14 simulation')
        plt.plot(np.logspace(14, 16.5, 100), dataDf_n_303, label='n 303K, 2e15 simulation')
        plt.legend()
    

    def SHR_objective_function2(self, params):
    ...
    def experimental_data_fit3(self, initial_guess=None):
        '''
        Minimize the lost function output by SHR_objective_function
        '''
        # define the bounds for parameters
        ...
        Ntn_max = 1e13
        # write into bounds
        bounds = [(sp1_min, sp1_max), (sp2_min, sp2_max), (Ntp_min, Ntp_max), (Ntn_min, Ntn_max)]

        # define the initial guess
        if initial_guess == None:
            initial_guess = [1e-14, 1e-15, 1e10, 1e12]

        # minimize the loss function
        print('Start minimization')
        result = minimize(self.SHR_objective_function2, x0=initial_guess, bounds=bounds, method='trust-constr')
        ...
        plt.scatter(self.dn_p_303_8_5e14, self.tau_p_303_8_5e14, label='p 303K, 8.5e14 experiment')
        plt.scatter(self.dn_n_303_2e15, self.tau_n_303_2e15, label='n 303K, 2e15 experiment')
        plt.plot(np.logspace(13, 15.5, 100), dataDf_p_303*10, label='p 303K, 8.5e14 simulation')
        plt.plot(np.logspace(14, 16.5, 100), dataDf_n_303*10, label='n 303K, 2e15 simulation')
        plt.legend()